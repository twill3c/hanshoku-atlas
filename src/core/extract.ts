 ...
 * 桁で違っていた。再始動して慣性最小を採ると 5 枚すべてが復元された。
 *
 * **訂正(実測 2026-09-01):** 当初ここには「慣性は良し悪しの**完全な**判別子として働く」と
 * 書いていた。**実物の JPEG を通して測り直したら、それは言い過ぎだった。**
 * 品質 95 の JPEG に通した合成木版 30 枚のうち、再始動 4 回で 8 枚が壊れる。
 * 再始動を 16 回に増やすと 4 枚まで減るが、**64 回にしても 4 枚のまま**である ——
 * つまり残りの 4 枚では、**慣性がより低い解のほうが真値と合わない**。
 * 慣性は良い判別子ではあるが、完全ではない。
 *
 * **既定を 4 のままにしている理由:** 16 にすれば失敗は半減するが 4 倍の時間がかかり、
 * それでも 0 にはならない。そして本物の版画は、この合成木版のように 8 画素ごとに
 * 色の境界があるわけではない —— **合成のほうが JPEG にとって最悪の入力**である。
 * ⑤ で測った誤差棒(本物の画像・再始動 4)は、この不安定さを含んだ値になっている。
 *
